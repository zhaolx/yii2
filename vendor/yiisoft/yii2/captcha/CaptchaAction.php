...
class CaptchaAction extends Action
{
    ...
    private function createLine() {
        for ($i=0;$i<6;$i++) {
            $color = imagecolorallocate($image->newImage,mt_rand(0,156),mt_rand(0,156),mt_rand(0,156));
            imageline($image->newImage,mt_rand(0,$this->width),mt_rand(0,$this->height),mt_rand(0,$this->width),mt_rand(0,$this->height),$color);
        }
        for ($i=0;$i<100;$i++) {
            $color = imagecolorallocate($image->newImage,mt_rand(200,255),mt_rand(200,255),mt_rand(200,255));
            imagestring($image->newImage,mt_rand(1,5),mt_rand(0,$this->width),mt_rand(0,$this->height),'*',$color);
        }
    }
